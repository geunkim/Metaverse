...
async def pool_conect():
    try:
        pool.set_protocol_version(PROTOCOL_VERSION)

        async with aiohttp.ClientSession() as session:
            async with session.request("GET",
                                       f'http://220.68.5.139:8001/connections') as resp:
                print(resp.status)
            ...
            async with session.request("GET",
                                       f'http://220.68.5.139:9000/genesis', headers=headers) as resp:
                print(resp.status)
                print(await resp.text())
...
async def proof_negotiation():
    try:
        ...
        pprint.pprint(genesis_file_path)
...
def main():
    ...
    loop.run_until_complete(pool_conect())
    loop.close()
